...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        #capsule score
        score += 10/(self.getScoreFor(currentGameState.getCapsules(), newPos, 'min')[0])
        ...
        score += 10/(self.getScoreFor(newFood.asList(), newPos, 'min')[0])

        #ghost score
        scared = [ghost.getPosition() for ghost in currentGameState.getGhostStates() if ghost.scaredTimer == 0]
        if len(scared) != 0:
            score -= 25**(3 - min(self.getScoreFor(scared, newPos, 'min')[0], 3))

        #adjusting food resource count & eating capsules (buffer for score)
...
def betterEvaluationFunction(currentGameState):
    ...
    DESCRIPTION:
    Implemented the A* search from Project 1.

    Although A* search isn't the best performing search,
    the maze has a fixed size (which isn't too large), and
    in such enviornment, I believe A* search will yield a better evaluation.
    """
    "*** YOUR CODE HERE ***"
...
def aStarSearch(state, heuristic):
    ...
    while(not temp.isEmpty()):
        ...
        for stateNext, locNext, countNext in problem.getSuccessors(state):
            ...
            temp.push((stateNext, loc + [locNext], tempCount), tempCount + heuristic(stateNext, problem))
