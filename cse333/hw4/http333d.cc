...
void GetPortAndPath(int argc,
  ...
  *port = atoi(argv[1]);
  ...
  //verify if path is readable
  struct stat err1;
  if(stat(argv[2], &err1) == -1)
	  Usage(argv[0]);
  if(!S_ISDIR(err1.st_mode))
	  Usage(argv[0]);
  
  *path = std::string(argv[2]);
  
  //verify all indicies are readable
  for(int i = 3; i < argc; i++){
	  struct stat err2;
	  if(stat(argv[i], &err2) == -1)
		  Usage(argv[0]);
	  if(!S_ISDIR(err2.st_mode))
		Usage(argv[0]);
	  indices->push_back(argv[i]);
  }
  
  //verify at least one indice
  if(indices->size() == 0)
	  Usage(argv[0]);
}