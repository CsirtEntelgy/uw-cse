...
public class NGramToNextChoicesMap {
    ...
    public void seenWordAfterNGram(NGram ngram, String word) {
        if (map.find(ngram) == null) map.insert(ngram, newInner.get());
        if (map.find(ngram).find(new AlphabeticString(word)) == null)
            map.find(ngram).insert(new AlphabeticString(word), 1);
        else {
            int count = map.find(ngram).find(new AlphabeticString(word)) + 1;
            map.find(ngram).insert(new AlphabeticString(word), count);
        }
    }

    /**
     * Returns an array of the DataCounts for this particular ngram. Order is not
     * specified.
     *
     * @param ngram
    ...
    public Item<String, Integer>[] getCountsAfter(NGram ngram) {
        ...
        if (map.find(ngram) == null)
            ...
            int counter = 0;
            temp = new Item[map.find(ngram).size()];
            Iterator<Item<AlphabeticString, Integer>> itr = map.find(ngram).iterator();
            while (itr.hasNext()) {
                Item<AlphabeticString, Integer> i = itr.next();
                temp[counter] = new Item<String, Integer>(i.key.toString(), i.value);
                counter++;
            }
        }
        return temp;
