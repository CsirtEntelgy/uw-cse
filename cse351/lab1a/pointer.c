...
int changeValue() {
  ...
  int temp = 0x1;
  temp = temp << 8; // shift by 8
  temp = temp + 0x5F;
  intPtr1 = intPtr1 + 5; // [5] is the 6th element
  *intPtr1 = temp;
  return intArray[5];
}
...
int withinSameBlock(int *ptr1, int *ptr2) {
  ...
  // shift 6 (ignoring the first 6 digits)
  // using binary + to join
  return !((~(b >> 6) + 1) + (a >> 6));
}
...
void selectionSort(int arr[], int arrLength) {
  ...
  for(a = 0; a < arrLength - 1; a++){
   min_index = a;
   for(b = a + 1; b < arrLength; b++){
      if(*(arrTemp + min_index) > *(arrTemp + b)){
         min_index = b;  
      }
   }
   swapInts(arrTemp + a, arrTemp + min_index); 
  }
}