...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        if(ballContainer.size() < maxVolume 
    ...
    public Iterator<Ball> getBallsFromSmallest() {
    	BallContainer result = new BallContainer();
    	BallContainer original = ballContainer;
    	while(original.size() != 0) {
    		Iterator<Ball> itr = original.iterator();
    		Ball b = itr.next();
    		while(itr.hasNext()) {
    			Ball temp = itr.next();
    			if(b.getVolume() > temp.getVolume())
    				b = temp;
    		}
    		result.add(b);
    		original.remove(b);
